(ns boeing_health.main
  "Kotodama WASM entrypoint for the Boeing_health clean-room actor (L4) — Clojure port.
  ...
  facts (`boeing_health.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "boeing_health")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"boeing_health.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boeing_health-compat" :tier tier :entities entities} 200])
...
(defn start! [] :boeing_health-compat/ready)
